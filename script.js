(function () {
  ...
  async function playScene(index) {
    ...
    // Show See Again button only on last scene
  ...
  sceneContainer.onclick = () => { if (!typing && currentScene < scenes.length - 1) goToScene(currentScene + 1); };
  seeAgainBtn.onclick = () => {
    seeAgainBtn.classList.remove('visible');
    nextBtn.style.display = 'inline-block';
    prevBtn.style.display = 'inline-block';
